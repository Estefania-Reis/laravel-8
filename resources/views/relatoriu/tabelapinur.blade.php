...
<hr>
    <h3 style="text-align: center">Relatoriu Estatistika Anual Produsaun Ikan Nursery Pescas e Aquicultura</h3>
  <table id="customers">
    <tr>
      <th>No</th>
    ...
    @endphp
      ...
      <tr>
      <td>{{ $no++}}</td>
      <td>{{ date('Y', strtotime($data->data)) }}</td>
      <td>{{ $data->total_ikan_oan_msex_atual }}</td>
      <td>{{ $data->total_ikan_oan_nmsex_atual }}</td>
      <td>{{ $data->total_ikan_oan_atual }}</td>
