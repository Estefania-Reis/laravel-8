...
<hr>
    <h3 style="text-align: center">Relatoriu Estatistika Anual Distribuisaun Fini Ikan Husi Pescas e Aquicultura</h3>
  <table id="customers">
    <tr>
      <th>No</th>
    ...
    <tr>
        <td>{{ $no++}}</td>
        <td>{{ date('Y', strtotime($raw->data)) }}</td>
        <td>{{ $raw->total_msex_dim }}</td>
        <td>{{ $raw->total_nmsex_dim }}</td>
        <td>{{ $raw->total_dim }}</td>
      </tr>  
    @endforeach
  </table>
