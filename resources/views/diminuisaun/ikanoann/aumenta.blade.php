...
<br><br><br>
                        ...
                        <form action="/insertikanoanmate" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                ...
                                <div class="mb-3 mr-3">
                                    ...
                                    <select class="form-select" name="nurseryn_id" aria-label="Default select example">
                                        @foreach ($dataikan as $data)
                                            <option selected value="{{ $data->id_ikannurseryn }}">{{ $data->id_ikannurseryn }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="mb-3 mr-3">
                                    <label for="exampleInputEmail1"class="form-label">Total Ikan Nursery N. M. Sex</label>
                                    <select class="form-select"  id="nurseryn_id" onChange="total_ikannurseryn(this.value)" aria-label="Default select example">
                                        <option value="0" selected>hili</option>
                                    @foreach ($dataikan as $data)
                                        <option value="{{ $data->total_ikan_oan }}">{{ $data->id_ikannurseryn }}</option>
                                    @endforeach
                                </select>
                                    <input type="text" name="total_nurseryn"  class="form-control" id="total_nurseryn" 
                            ...
                            <div class="row">
                                ...
                                <div class="mb-3 mr-3">
                                    <label for="exampleInputEmail1"class="form-label">Total Ikan Distribuidu N. M. Sex</label>
                                    <select class="form-select"  id="distribuisaun_id" onChange="total_distribuisaun(this.value)" aria-label="Default select example">
                                        <option value="0" selected>hili</option>
                                    @foreach ($datadis as $data)
                                        <option value="{{ $data->total_n_sex }}">{{ $data->id_distribuisaun }}</option>
                                    @endforeach
                                </select>
    ...
    <script>
        function total_ikannurseryn(val){
            var t=val;
            document.getElementById("total_nurseryn").value=t;
        }
    </script>
    <script>
    ...
    <script>
        ...
        let total_nurseryn = parseFloat($('#total_nurseryn').val())
        let total_distribuisaun = parseFloat($('#total_distribuisaun').val())
        let rezultadu = total_nurseryn - total_distribuisaun
    ...
    $(document).on('blur','#total_nurseryn', function(){
        let total_nurseryn = parseFloat($('#total_nurseryn').val())
        let total_distribuisaun = parseFloat($('#total_distribuisaun').val())
        let rezultadu = total_nurseryn - total_distribuisaun
        $('#total_diminuisaun').val(rezultadu)
